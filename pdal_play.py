...
for vdatum in vdatums:
    ...
    for k, v in meta_dict['metadata']['readers.las'].items():
        if 'vlr_' in k:
            print('{} ({})'.format(k, v['record_id']))
            print('description: {}'.format(v['description']))
            print('user_id: {}'.format(v['user_id']))
            print('data: {}'.format(v['data']))
            print('data: {}'.format(base64.b64decode(v['data'].encode())))
            print()
